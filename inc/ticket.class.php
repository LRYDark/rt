...
class PluginRtTicket extends CommonDBTM {
   ...
   **/
   static function showForTicket(Ticket $ticket) { // formulaire sur le ticket
      ...
      if(Session::haveRight("plugin_rt_rt", READ) || Session::haveRight("plugin_rt_rt", PURGE) || Session::haveRight("plugin_rt_rt", UPDATE)){
         if ($number) {
            $out .= "<div class='spaced'>";
            ...
            if(Session::haveRight("plugin_rt_rt", UPDATE) || Session::haveRight("plugin_rt_rt", PURGE)){
               if ($canedit) { // fonction massiveactions (utiliser dans la partie temps de trajet sur le ticket)
                  $out .= Html::getOpenMassiveActionsForm('mass'.__CLASS__.$rand);
                     if( Session::haveRight("plugin_rt_rt", PURGE)){
                        $massiveactionparams =  [
   ...
   static function updateroutetime(TicketTask $item){ // fonction de la mise à jour du temps de trajet (appel a la fonction lorsque l'on modifie une tache)
      ...
      if(Session::haveRight("plugin_rt_rt", UPDATE)){
         ...
         Session::addMessageAfterRedirect(
            __("Vous n'avez pas les droits pour mettre à jour le temps de trajet", 'rt'),
            true,
            ERROR
         );
      }
                              ...
                              // Téléphone / email
                                 echo "<tr>";
                                    ...
                                    echo "</td>";
                                       ...
                                       echo "<td>";
                                          $config = new PluginRtConfig();
                                          if ($config->fields['mail'] == 1){
                                             echo "<label for='email'>Courriels</label>&nbsp<input type='checkbox' id='mailto' name='mailto' value='false' onchange='chState(this)'><br>";
                                          }else{
                                             echo "<label for='email'>Courriels</label><input type='checkbox' id='mailto' name='mailto' value='false' style='visibility: hidden;' disabled><br>";
                                          }  
      ...
      if($EntitieAddress == 0 && $ticketId != 0 && !empty($ticketId)){
         ...
         if(Session::haveRight("plugin_rt_affichage", READ)){
            ...
            if (!empty($complement) || !empty($address)) {
               ...
               // 1) Construire la destination texte (utilisée par le modal ORS)
               ...
               // 4) Badge GLPI propre (sans row/d-flex qui cassent la ligne)
               $entitie = "<span class='glpi-badge form-field mb-2' style='margin-top:3px; display:block'>"
                        . $inline
                        . "</span>";
